...
type EntrarFormValues = z.input<typeof loginSchema>;
type EntrarPayload = z.output<typeof loginSchema>;

type EntrarResponse = {
  error?: string;
  user: AuthenticatedUser;
};
type EntrarResponsePayload = Omit<EntrarResponse, "user"> & {
  user?: AuthenticatedUser | null;
};

class EntrarError extends Error {
  /**
 ...
 *
 * @param values Entrar form values.
 * @returns Authenticated user payload.
 */
const login = async ({ email, password, remember }: EntrarPayload) => {
  ...
  const payload = (await response.json().catch(() => ({}))) as EntrarResponsePayload;

  if (!response.ok) {
    throw new EntrarError(payload.error ?? "Falha ao entrar. Verifique seu email e sua senha.");
  }

  if (!payload.user) {
    throw new EntrarError("A resposta de login não incluiu um usuário autenticado.");
  }

  return {
    ...payload,
    user: payload.user,
  } satisfies EntrarResponse;
};
